...
2. **Mimic Tooth.**  Its multiplier changes the levelgen mimic decisions in
   ...
   analyzer now includes seed-reachable **Mimic Tooth +0…+3** routes from the
   seed's Catalyst/transmutation path: the level-specific multiplier is
...
baseline path it actually simulated; each labelled Mimic Tooth condition instead
replays that Tooth path and reports its single resulting class
...
(`crates/spd-core/src/level/state.rs:322-435`). It also automatically discovers
the seed-reachable held Mimic Tooth +0…+3 conditions. Other stateful
