...
- **SEED-ONLY-REPORTING** — Public reports and seed-finder matches must expose concrete item identity, level, curse, enchantment, quantity, or placement only when that fact is invariant from the seed and pinned deterministic generation state. If player runtime/history (inventory, trinkets, optional branches, combat drops, prior generator-deck consumption, or other choices) can change a field, redact that concrete field and report only source-backed stable constraints such as category, tier/range, forced properties, and clearly labeled conditional player-state effects. Keep exact runtime-sensitive values internal for parity fixtures; never let map markers/heaps, WASM, search evidence, or UI bypass this rule.
