...
- **SEED-ONLY-REPORTING** — Public reports and seed-finder matches may assert only facts proven invariant from the seed alone for the pinned game version. Never expose concrete item identity, existence, level, curse, enchantment, quantity, heap type, placement, room/layout metadata, or map cell when player runtime/history (inventory, trinkets, challenges, settings, optional branches, combat drops, prior generator/deck consumption, or other choices) can change it. Report only source-backed invariant constraints such as a reward category, tier/range, or forced property; describe player-state effects solely as clearly labeled conditions, never as the run's concrete outcome. Prefer omitting an unsafe field or map over publishing a best-effort snapshot. Keep exact runtime-sensitive values internal for parity fixtures; map markers/heaps, WASM, search evidence, and UI must use the same public projection.
