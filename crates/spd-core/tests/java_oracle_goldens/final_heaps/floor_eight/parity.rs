...
pub(super) fn assert_map(map: &FloorMap, expected: &OracleFloor) {
    assert_eq!(
        map.tiles,
        *expected.terrain.as_ref().expect("floor-8 terrain"),
        "HKT floor-8 terrain"
    );
    assert_eq!(
        map.discoverable,
        *expected
            .discoverable
            .as_ref()
            .expect("floor-8 discoverable mask"),
        "HKT floor-8 discoverable mask"
    );
    assert_eq!(
        map.tile_variance,
        *expected
            .tile_variance
            .as_ref()
            .expect("floor-8 tile variance"),
        "HKT floor-8 tile variance"
    );
    ...
    let actual_heaps: Vec<_> = map
        ...
        .collect();
    assert_eq!(actual_heaps, expected.final_heaps, "HKT floor-8 heaps");
    ...
    let actual_mobs: Vec<_> = map
        ...
        .collect();
    assert_eq!(actual_mobs, expected.final_mobs, "HKT floor-8 mobs");
    ...
    assert_eq!(
        actual_transitions,
        *expected.transitions.as_ref().expect("floor-8 transitions"),
        "HKT floor-8 transitions"
    );
    ...
    assert_eq!(
        actual_traps,
        *expected.traps.as_ref().expect("floor-8 traps"),
        "HKT floor-8 traps"
    );
    ...
    assert_eq!(
        actual_plants,
        *expected.plants.as_ref().expect("floor-8 plants"),
        "HKT floor-8 plants"
    );
    ...
    assert_eq!(
        actual_blobs,
        *expected.blobs.as_ref().expect("floor-8 blobs"),
        "HKT floor-8 blobs"
    );
}
