...
pub fn analyze_seed(input: &str, floors: u32) -> Result<SeedReport, AnalyzeError> {
    ...
    Ok(SeedReport {
        ...
        message: Some(
            "Partial analysis: layout builder + special/secret-room prizes (incl. pit/garden/wells/maze/summoning/chest-chasm) + shop/crystal loot (approx.) + Ghost/Wandmaker/Blacksmith/Imp quest rewards + water/grass/trap painter + paintDoors merge/Graph (approx. room geometry) + main createItems drops. Full createMobs, full room geometry paint, Maze.generate, and figure-eight builder still incomplete — results may not match the game yet."
                .to_string(),
        ),
    })
}
