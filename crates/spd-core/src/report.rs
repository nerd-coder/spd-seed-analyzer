...
use crate::trinkets::{ProfileError, TrinketSelectionReport};
...
/// A replayed, user-meaningful run condition.
///
/// These outcomes are generated by the analyzer rather than supplied by the
/// UI. They are intentionally limited to first-generation branches supported
/// by the current partial port.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ModeledOutcome {
    /// Concise condition shown alongside this result.
    pub condition: String,
    /// Important qualification for interpreting this branch.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
pub struct SeedReport {
    ...
    pub floors: Vec<FloorReport>,
    /// Replayed outcomes for the finite set of run conditions the partial
    /// analyzer currently models. `floors` above remains the
    /// conservative seed-only projection used by search.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub modeled_outcomes: Vec<ModeledOutcome>,
    /// Scope and caveats for `modeled_outcomes`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
