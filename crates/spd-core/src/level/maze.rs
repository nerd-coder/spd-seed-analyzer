//! Shared pinned SPD `Maze.generate` feature.
...
pub(crate) fn generate_for_room(room: &Room, doors: &[Point]) -> (Vec<bool>, i32, i32) {
...
fn random_filled(maze: &[bool], width: i32, height: i32) -> Option<(i32, i32)> {
    // Boundary walls guarantee success for valid room dimensions. This only
    // protects malformed WASM callers from an endless rejection loop.
...
pub(crate) fn index(width: i32, x: i32, y: i32) -> usize {
    (x + y * width) as usize
}
